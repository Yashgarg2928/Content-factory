...
def generate_text(prompt, GOOGLE_API_KEY):
    """
...
def gemini_fn(data):
    script, index, base_folder, reel_no, google_api_key, scripts = data

    print(f"▶️ Processing reel {reel_no} in process {os.getpid()}")
    ...
    **[storyName]+[ReelNumber]+[ImageNumber]**  
    ...
    image_prompts = generate_text(image_making_prompt,google_api_key[scripts.index(script)%8])
    ...
    audio_script = generate_text(audio_prompt,google_api_key[scripts.index(script)%8])
    ...
    csv_file = generate_text(csv_making_prompt,google_api_key[scripts.index(script)%8])
...
if __name__ == "__main__":
    ...
    data_list = [
        (scripts_all[i], i, base_folder, j, google_api_key_list, scripts_all)
        for j, i in enumerate(range(2, len(scripts_all)), start=1)
    ]

    with ProcessPoolExecutor(max_workers=8) as executor:
        results = executor.map(gemini_fn, data_list)
    print("ALL REEL DONE ")
